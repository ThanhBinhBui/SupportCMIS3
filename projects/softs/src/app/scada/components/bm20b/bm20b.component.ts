...
@Component({
  selector: 'app-bm20b',
  templateUrl: './bm20b.component.html',
  styleUrls: ['./bm20b.component.css', '../../../../styles.css']
})
export class Bm20bComponent implements OnInit {
  loading: boolean;
  lstMain: any[];
  TO_DATE: string;
  login: any;
  ...
  constructor(private scadaService: ScadaService,
    ...
    this.breadcrumbService.setItems([
      { label: 'BM20B - Báo cáo điều khiển xa thiết bị trung áp', routerLink: 'scada/bm20b' }
    ]);
  }
  ...
  search() {
    ...
    this.scadaService.spc_Scada_BM20BBaoCaoDKX({ DVQL: this.MA_DVIQLY, DN: DN }).subscribe(
      data => {
        this.lstMain = data;
        this.loading = false;
      }, error => {
  ...
  exportExcel() {
    const title = 'BÁO CÁO ĐIỀU KHIỂN XA THIẾT BỊ TRUNG ÁP';
    let workbook = this.reportService.showReportBM20b(
      this.lstMain,
      this.TO_DATE,
      title,
      title,

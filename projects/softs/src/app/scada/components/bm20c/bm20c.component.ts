...
@Component({
  selector: 'app-bm20c',
  templateUrl: './bm20c.component.html',
  styleUrls: ['./bm20c.component.css', '../../../../styles.css']
})
export class Bm20cComponent implements OnInit {
  loading: boolean;
  lstMain: any[];
  TO_DATE: string;
  login: any;
  ...
  constructor(private scadaService: ScadaService,
    ...
    this.breadcrumbService.setItems([
      { label: 'BM20C - Tổng hợp kết quả thao tác xa TBA KNT từ hệ thống SCADA', routerLink: 'scada/bm20c' }
    ]);
  }
  ...
  search() {
    ...
    this.scadaService.spc_Scada_BM20CTongHopKQThaoTacXa({ DVQL: this.MA_DVIQLY, DN: DN }).subscribe(
      data => {
        this.lstMain = data;
        this.loading = false;
      }, error => {
  ...
  exportExcel() {
    const title = 'TỔNG HỢP KẾT QUẢ THAO TÁC XA TBA KNT TỪ HỆ THỐNG SCADA';
    let workbook = this.reportService.showReportBM20c(
      this.lstMain,
      this.TO_DATE,
      title,
      title,

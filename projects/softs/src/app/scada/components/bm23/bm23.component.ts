...
@Component({
  selector: 'app-bm23',
  templateUrl: './bm23.component.html',
  styleUrls: ['./bm23.component.css']
})
export class Bm23Component implements OnInit {
  ...
  constructor(private hisMessageService: HISMessageService,
    ...
    this.breadcrumbService.setItems([
      ...
      { label: 'BM23 - Tổ thao tác lưu động và thao tác tại TBA', routerLink: 'thietbi/bm23' }
    ]);
  }
  ...
  search() {
    ...
    // this.yearCurrent = year;
    // console.log({ month, year });
    this.hisMessageService.getBM23ThaoTacTaiTBA({ month, year }).subscribe(data => {
      if(data!==null){
        console.log(data);
  ...
  saveItem() {
    ...
    debugger;
    this.hisMessageService.insertBM23ThaoTacTaiTBA(Object.assign(this.mainCurrent, { NGUOI_TAO: JSON.parse(localStorage.getItem('user')).FULL_NAME })).subscribe(result => {
      console.log(result);
  ...
  exportExcel() {
    const fileName = 'BM23';
    const title = 'TỔ THAO TÁC LƯU ĐỘNG VÀ THAO TÁC TẠI TBA    ';
    let workbook = this.reportService.showReportBM23(
      this.listMain,
      fileName,
      title
    );

...
@Component({
  selector: 'app-bm24',
  templateUrl: './bm24.component.html',
  styleUrls: ['./bm24.component.css']
})
export class Bm24Component implements OnInit {
  listMain: any[];
  listMainCa: any[];
  mainListCa: any[];
  mainCaCurrent: any;
  ...
  ngOnInit(): void {
    ...
    this.listUser = [
      {
        "USER_ID": 1160,
        "FULL_NAME": "Trịnh Gia Hồng Phúc",
        "EMPLOYEE_ID": 1494000000000019,
        "ORG_CODE": "F02D06",
        "GENDER": 1,
        "STATUS": 1
      },
      ...
      {
        "USER_ID": 1166,
        "FULL_NAME": "Nguyễn Đình Trình",
        "EMPLOYEE_ID": 1494000000000026,
        "ORG_CODE": "F02D06",
        "GENDER": 1,
        "STATUS": 1
      },
      {
        "USER_ID": 1169,
        "FULL_NAME": "Đồng Nguyễn Công Danh",
        "EMPLOYEE_ID": 1494000000000030,
        "ORG_CODE": "F02D06",
        "GENDER": 1,
        "STATUS": 1
      }
    ...
    this.FROM_DATE_CA = this.datePipe.transform(firstDay, 'dd/MM/yyyy');
    this.TO_DATE_CA = this.datePipe.transform(now, 'dd/MM/yyyy');
    this.login = JSON.parse(localStorage.getItem('user'));
    this.selectedCategory = "SU_KIEN";
    this.listCategory = [
      { label: 'Sự kiện trong ca trực', value: "SU_KIEN" },
      { label: 'Tình trạng vận hành các thiết bị', value: "TINH_TRANG" },
      { label: 'Các vấn đề lưu ý trong phiên', value: "LUU_Y" }
    ];
    this.getAllCaByNgay(this.FROM_DATE_CA);
    this.searchCa();
  }
  ...
  onChangeDate(d) {
    this.getAllCaByNgay(this.convertStringToDate(d));
  }
  getCa(startDate, endDate) {
    this.loadingCa = true;
    this.hisMessageService.getSoNhatKyVanHanhCa({ startDate, endDate }).subscribe(data => {

      this.listMainCa = data;
      console.log(data);
      this.loadingCa = false;
    });
  }
  searchCa() {
    if (this.FROM_DATE_CA !== undefined && this.TO_DATE_CA !== undefined && this.FROM_DATE_CA !== null && this.TO_DATE_CA !== null && this.FROM_DATE_CA !== '' && this.TO_DATE_CA !== '') {
      this.getCa(this.convertStringToDate(this.FROM_DATE_CA), this.convertStringToDate(this.TO_DATE_CA))
    }
    else {
      this.messageService.add({ severity: 'warn', summary: 'Cảnh báo', detail: 'Vui lòng chọn thời gian tìm kiếm' });
    }
  }
  getDataDetailByCa() {

    if (this.selectedCaTypingCurrent === null) {
      this.messageService.add({ severity: 'warn', summary: 'Cảnh báo', detail: 'Vui lòng chọn ca để xem dữ liệu' });
    }
    else {
      this.getSuKien(this.selectedCaTypingCurrent.ID);
      this.getTinhTrang(this.selectedCaTypingCurrent.ID);
      this.getLuuY(this.selectedCaTypingCurrent.ID);
    }
  }
  getAllCaByNgay(ngay) {
    this.hisMessageService.getBM24SoNhatKyVanHanhCa({ ngay: ngay }).subscribe(
      data => {
        console.log(data);
        if (data !== null && data.length !== 0) {

          this.listCa = data.map(item => {
  ...
  editItem(item: any, category: any) {
    ...
    if (category === 'SU_KIEN') {
      ...
      // debugger;
      // this.mainCurrent.END_TIME = this.datePipe.transform(new Date(item.END_TIME), 'dd/MM/yyyy');
      // debugger;
      // this.mainCurrent.START_TIME = this.datePipe.transform(new Date(item.START_TIME), 'dd/MM/yyyy');
      // debugger;
      // this.mainCurrent.END_TIME = this.datePipe.transform(item.END_TIME, 'dd/MM/yyyy');
      // this.mainCurrent.START_TIME = this.datePipe.transform(item.START_TIME, 'dd/MM/yyyy');
      // this.mainCurrent.END_TIME = new Date(item.END_TIME);
      ...
      this.isUpdate = true;
      this.dialog = true;
      //this.selectedTBASPC = this.tbaSPCs.filter(i=>i.MA_TRAM=llTBA.MA_TRAM)[0];
    } else if (category === 'TINH_TRANG') {
      this.contentTinhTrang = item.CONTENT;
      this.idTinhTrang = item.ID;
      this.isUpdate = true;
      this.dialog = true;
    } else if (category === 'LUU_Y') {
      this.contentLuuY = item.CONTENT;
      this.idLuuY = item.ID;
      this.isUpdate = true;
      this.dialog = true;
    }
  ...
  deleteItem(item: any) {
    if (this.selectedCategory === 'SU_KIEN') {
      ...
      //this.selectedTBASPC = this.tbaSPCs.filter(i=>i.MA_TRAM=llTBA.MA_TRAM)[0];
    } else if (this.selectedCategory === 'TINH_TRANG') {
      this.confirmationService.confirm({
        message: 'Bạn có chắc muốn xóa thông tin tình trạng ?',
        header: 'Confirm',
        icon: 'pi pi-exclamation-triangle',
        accept: () => {
          this.hisMessageService.deleteSoNhatKyVanHanhTinhTrang({ ID: item.ID }).subscribe(result => {
            if (result.result === 'SUCCESS') {
              this.getTinhTrang(this.selectedCa);
              this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'Đã xoá thành công', life: 3000 });
            }
          })
        }
      });
    } else if (this.selectedCategory === 'LUU_Y') {
      this.confirmationService.confirm({
        message: 'Bạn có chắc muốn xóa thông tin lưu ý ?',
        header: 'Confirm',
        icon: 'pi pi-exclamation-triangle',
        accept: () => {
          this.hisMessageService.deleteSoNhatKyVanHanhLuuY(({ ID: item.ID })).subscribe(result => {
            if (result.result === 'SUCCESS') {
              this.getLuuY(this.selectedCa);
              this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'Đã xoá thành công', life: 3000 });
            }
          })
        }
  ...
  deleteItemCa(item: any) {
    this.confirmationService.confirm({
      message: 'Bạn có chắc muốn xóa ca trực?',
      header: 'Confirm',
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.hisMessageService.deleteSoNhatKyVanHanhCa(item).subscribe(result => {
          if (result.result === 'SUCCESS') {
            this.searchCa();
            this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'Đã xoá thành công', life: 3000 });
          }
  ...
  saveItem() {
    ...
    if (this.selectedCategory === 'SU_KIEN') {
      // debugger;
      if (this.selectedCaTypingCurrent !== undefined && this.selectedCaTypingCurrent !== null) {
        debugger;
        this.mainCurrent.DONVI_QUANLY = this.selectedDonViQuanLy;
        this.mainCurrent.DONVI_XULY = this.selectedDonViQuanLyNgoai;
        this.mainCurrent.ID_CA = this.selectedCaTypingCurrent.ID;
        if(this.mainCurrent.END_TIME===undefined||this.mainCurrent.END_TIME===null){
          this.mainCurrent.END_TIME===new Date('0001-01-01T00:00:00');
        }
        ...
        if (check === 'SUCCESS') {
          if (this.isUpdate === false) {
            debugger;
            this.hisMessageService.insertSoNhatKyVanHanh(this.mainCurrent).subscribe(result => {
              console.log(result);
              if (result.result === 'SUCCESS') {
        ...
        // debugger;
        this.messageService.add({ severity: 'error', summary: 'Lỗi', detail: 'Vui lòng chọn ca', life: 3000 });
      }
    } else if (this.selectedCategory === 'TINH_TRANG') {
      // debugger;
      if (this.selectedCaTypingCurrent !== undefined && this.selectedCaTypingCurrent !== null) {
        const check = this.validateFormTinhTrang({ ID_CA: this.selectedCaTypingCurrent.ID, CONTENT: this.contentTinhTrang });
        debugger;
        if (check === 'SUCCESS') {
          if (this.isUpdate === false) {
            this.hisMessageService.insertSoNhatKyVanHanhTinhTrang({ ID_CA: this.selectedCaTypingCurrent.ID, CONTENT: this.contentTinhTrang }).subscribe(result => {
              console.log(result);
              if (result.result === 'SUCCESS') {
                this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'Đã thêm thành công', life: 3000 });
                this.getTinhTrang(this.selectedCaTypingCurrent.ID);
            ...
            debugger;
            this.hisMessageService.updateSoNhatKyVanHanhTinhTrang({ ID: this.idTinhTrang, CONTENT: this.contentTinhTrang }).subscribe(result => {
              if (result.result === 'SUCCESS') {
                this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'Đã cập nhật thành công', life: 3000 });
                this.getTinhTrang(this.selectedCaTypingCurrent.ID);
                this.selectedDonViQuanLy = null;
      ...
      debugger;
      if (this.selectedCaTypingCurrent !== undefined && this.selectedCaTypingCurrent !== null) {
        const check = this.validateFormTinhTrang({ ID_CA: this.selectedCaTypingCurrent.ID, CONTENT: this.contentLuuY });
        debugger;
        if (check === 'SUCCESS') {
          if (this.isUpdate === false) {
            this.hisMessageService.insertSoNhatKyVanHanhLuuY({ ID_CA: this.selectedCaTypingCurrent.ID, CONTENT: this.contentLuuY }).subscribe(result => {
              console.log(result);
              if (result.result === 'SUCCESS') {
                this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'Đã thêm thành công', life: 3000 });
                this.getLuuY(this.selectedCaTypingCurrent.ID);
            ...
            //debugger;
            this.hisMessageService.updateSoNhatKyVanHanhLuuY({ ID: this.idLuuY, CONTENT: this.contentLuuY }).subscribe(result => {
              if (result.result === 'SUCCESS') {
                this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'Đã cập nhật thành công', life: 3000 });
                this.getLuuY(this.selectedCaTypingCurrent.ID);
                this.selectedDonViQuanLy = null;
  ...
  saveItemCa() {
    ...
    if (this.selectedCaCurrent !== undefined && this.selectedCaCurrent !== null) {
      this.mainCaCurrent.ID_CA = this.selectedCaCurrent.ID;
      const check = this.validateFormCa(this.mainCaCurrent);
      debugger;
      if (check === 'SUCCESS') {
        if (this.isUpdateCa === false) {
          debugger;
          this.hisMessageService.insertCa({ ...this.mainCaCurrent, NGAY: this.mainCaCurrent.NGAY }).subscribe(result => {
            console.log(result);
            if (result.result === 'SUCCESS') {
              this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'Đã thêm thành công', life: 3000 });
              this.hideDialogCa();
              this.searchCa();
              // this.getCa(this.selectedCa);

            }
          });
        } else {
          debugger;
          this.hisMessageService.updateSoNhatKyVanHanh(this.mainCaCurrent).subscribe(result => {
            if (result.result === 'SUCCESS') {
              this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'Đã cập nhật thành công', life: 3000 });
              this.getSuKien(this.selectedCa);
              this.selectedDonViQuanLy = null;
  ...
  selectedCaTyping(main) {
    this.selectedCaTypingCurrent = main;
    this.getDataDetailByCa();
  }
